...
public class Tools {
    ...
    public static String argsToString(String[] args, int begin){
        return String.join(" ", Arrays.asList(args).subList(begin, args.length));
    }
    ...
    public static boolean isBotAdmin(User user) {
        if (BOT_ADMINS.contains(user.getId())){
            return true;
        } else {
            return false;
        }
